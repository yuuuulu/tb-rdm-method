#########################################
## Simulation with powerful predictors ##
#########################################
...
hhsim = function(data,tpar,beta,apar,pre_severe){
  n <- nrow(data)
  transmat <- mat.or.vec(n,n)
  datemat <- mat.or.vec(n,n)
  data$id <- 1:nrow(data)
  lambdac <- tpar['theta']*(data$adult*tpar['adult']+(1-data$adult))*(data$female*tpar['female']+(1-data$female))*
    (data$burden*tpar['burden']+(1-data$burden))*(data$ses*tpar['ses']+(1-data$ses))*(data$cough*tpar['cough']+(1-data$cough))
  ...
  if(n==1){
    ...
    if (any(trans == 2)){
      ...
      actind <- which(date==t)
      data[tail(actind,n=1),]$severity <-  rbinom(1,1,prob = pre_severe)
      ind <- 1:nrow(data)
      ...
      while(length(actind) <= n){
        ...
        sleep <- data[receiver,]$sleep
        time <- data[receiver,]$time
        severity <- data[sender,]$severity
        lambdah <- tpar['theta']*n^(beta)*(crowd*tpar['crowd']+(1-crowd))*(adult*tpar['adult']+(1-adult))*(female*tpar['female']+(1-female))*
          (sleep*tpar['sleep']+(1-sleep))*(time*tpar['time']+(1-time))*(severity*tpar['severity']+(1-severity))
...
tbsim = function(hhno,hhmin,hhmean,tpar,beta,apar,prevalence,munsize){
  ...
  mybeta <- beta
  myactivep <- apar
  names(mypar) <- c('theta','adult','female','crowd','sleep','time','severity','burden','ses','cough')
  names(myactivep) <- c('baseline','adult','female')
  names(prevalence) <- c('adult','female','crowd','sleep','time','severity','cough')
  ...
  burden <- numeric(np)
  burden[which(mun=='CARIACICA')] = 1;burden[which(mun=='SERRA')] = 0
  burden[which(mun=='VITORIA')] = 0;burden[which(mun=='VILA VELHA')] = 1
  ses <- numeric(np)
  ses[which(mun=='CARIACICA')] = 0;ses[which(mun=='SERRA')] = 1
  ses[which(mun=='VITORIA')] = 0;ses[which(mun=='VILA VELHA')] = 1
  cough <- rbinom(np,1,prob = prevalence['cough'])
  sleep <- rbinom(np,1,prob = prevalence['sleep'])
  time <- rbinom(np,1,prob = prevalence['time'])
  date <- rep(NA,np)
  severity <- rep(NA,np)
  dat <- cbind.data.frame(hhid,mun,crowd,female,adult,burden,ses,cough,sleep,time,severity,date,stringsAsFactors=FALSE)
  names(dat) <- c('hhid','mun','crowd','female','adult','burden','ses','cough','sleep','time','severity','date')
  dat <- split(dat,dat$hhid)
  output <- lapply(dat,'hhsim',tpar=mypar,beta=mybeta,apar=myactivep,pre_severe = prevalence['severity'])
  ...
  output <- output%>%ungroup(hhid)
  output <- output%>%select(-c("date","id"))
  return(output)
}
...
cl <- makeCluster(P)
clusterEvalQ(cl, {rbinom <- Vectorize(rbinom,c("prob"));library(tidyverse);library(hhcsim)})
clusterExport(cl,c("tbsim","hhsim"))
plmcmc <- function(iter){
  result <- tbsim(hhno=1500,hhmin=4,hhmean = 6.5,tpar=c(0.03,1.2,1,1.5,5.5,2,2.8,3.5,3,2.5),beta=0,apar=c(0.1,1,1),prevalence = c(0.7,0.5,0.4,0.1,0.3,0.5,0.2),munsize = rep(0.25,4))
  dat <- result%>%filter(hhc==1)
  mypar <- c(runif(1,0,1),exp(runif(9,0,1)))
  m <- mcmc(dat,iter,mypar,0.1,0.2)
  return(m)
}

out <- clusterApply(cl, rep(220000,250), fun = plmcmc)
out1 <- lapply(out,function(x) x[seq(20001,220000,by=20),])
saveRDS(out1,'powersim.rds')
stopCluster(cl)
